...
- ✅ Plot feature correlation heatmap and save to `outputs/`
...
- ✅ Plot global feature importance (SHAP bar chart) — saved to `outputs/shap_feature_importance.png`
- ✅ Plot SHAP summary plot — saved to `outputs/shap_summary.png`
...
- ✅ Save confusion matrix plot for best model to `outputs/` — `outputs/confusion_matrix.png` (generated by `04_results.ipynb`)
- ✅ Save ROC curve plot to `outputs/` — `outputs/roc_curve.png` (with Youden-J optimal threshold marked)
...
- ✅ `docs/setup.md` — dependency list realigned to `pyproject.toml` (added numpy/matplotlib/seaborn/shap/ipykernel, removed stale kagglehub; optional xgboost/lightgbm/imbalanced-learn noted); refreshed verify-install check and project structure
...
- ✅ Set up CI (GitHub Actions) — `.github/workflows/ci.yml` runs pytest on push/PR to main
...
- ✅ Add a SQL layer — `sql/churn_analysis.sql` + `scripts/sql_analysis.py` run DuckDB queries directly on the raw CSV
...
- ✅ Quantify business impact — `BUSINESS_ANALYSIS.md` translates churn reduction into retained revenue and ROI assumptions

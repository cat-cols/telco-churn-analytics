...
def validate_schema(data: pd.DataFrame) -> None:
    ...
    required_columns = set(
        NUMERICAL_COLUMNS + CATEGORICAL_COLUMNS + [IDENTIFIER_COLUMN, TARGET_COLUMN]
    )
...
def preprocess_data():
    ...
    X_train_scaled_df = pd.DataFrame(
        data=X_train_scaled, index=X_train.index, columns=NUMERICAL_COLUMNS
    )

    X_test_numerical: Any = X_test[NUMERICAL_COLUMNS]
    X_test_scaled: Any = scaler.transform(X_test_numerical)
    X_test_scaled_df = pd.DataFrame(
        data=X_test_scaled, index=X_test.index, columns=NUMERICAL_COLUMNS
    )
    ...
    X_test_encoded_df = pd.DataFrame(
        data=X_test_encoded, index=X_test.index, columns=encoder.get_feature_names_out()
    )

    # Combine processed features
    X_train_id: Any = X_train[IDENTIFIER_COLUMN]
    X_train_processed = pd.concat(
        [X_train_id, X_train_scaled_df, X_train_encoded_df], axis=1
    )

    X_test_id: Any = X_test[IDENTIFIER_COLUMN]
    X_test_processed = pd.concat(
        [X_test_id, X_test_scaled_df, X_test_encoded_df], axis=1
    )
